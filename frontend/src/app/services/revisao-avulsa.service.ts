...
export class RevisaoAvulsaService {
  ...
  /**
   * `Authorization: Bearer <access_token>` da sessão do Supabase Auth — único
   * mecanismo de acesso à API desde D-25 (a `X-API-Key` foi aposentada como
   * porta de entrada). Sem sessão o header sai vazio e o backend devolve 401,
   * que é o comportamento correto: a rota já exige login pelo `authGuard`, então
   * chegar aqui sem token significa sessão expirada no meio do uso.
   */
