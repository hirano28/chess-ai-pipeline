...
| `resumo_partida` | **258** |
| `fila_treino_espacado` | **716** — 658 do dono principal (agendada até 17/11) e 58 do segundo perfil (até 29/09). **16 são cards de trecho (EROSAO, D-66)**: 15 entraram na primeira execução da população com o recurso, todos no segundo perfil, espalhados pela cota de 2 por dia (22/09 a 29/09); o dono principal não recebeu nenhum porque a fila dele está cheia até o horizonte ("fila cheia até 18/11"), e o 16º é o card de verificação inserido à mão para ele. A válvula do D-64 continua em vigor. **712 dos 716 nunca foram respondidos** — o gargalo real do recurso não é o tamanho da fila, é o hábito de responder. **D-82 atacou as duas causas de interface disso**: a tela deixou de abrir com o tamanho do atraso (agora mostra `TREINO_META_DIARIA`, default 5) e o lance passou a ser respondido por clique no tabuleiro, não digitado. O número acima é a linha de base — a medir de novo depois de algumas semanas de uso real |
...
"Arte do Ataque no Xadrez" de Vukovic (D-78 sem capítulo, depois
reclassificado no D-80 — sumário e corpo usam formatos de capítulo
inconsistentes entre si, resolvido localizando os 12 capítulos reais por
tamanho de fonte no PDF em vez de padrão de texto, 39 conceitos gerados
depois) e "The Complete Manual of Positional Chess Vol 1" de Sakaev/Landa
...
isolado" (D-79) — categoria saltou de 4 para 33 conceitos citáveis sem
